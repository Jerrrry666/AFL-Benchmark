...
def create_sample_configs():
    ...
    configs = [
        {
            'name'  : 'uniform_label_uniform_feature.yaml',
            'config': {
                'partition'        : 'dual',
                'client_num'       : 20,
                'dir_path'         : 'domainnet',
                ...
                'label_partition'  : 'uni',
                'label_alpha'      : 10000,
                'feature_partition': 'uni',
                'feature_alpha'    : 10000,
            }
        },
        {
            'name'  : 'dirichlet_label_uniform_feature.yaml',
            'config': {
                'partition'        : 'dual',
                'client_num'       : 20,
                'dir_path'         : 'domainnet',
        ...
        {
            'name'  : 'uniform_label_dirichlet_feature.yaml',
            'config': {
                'partition'        : 'dual',
                'client_num'       : 20,
                'dir_path'         : 'domainnet',
                ...
                'label_partition'  : 'uni',
                'label_alpha'      : 10000,
                'feature_partition': 'dir',
                'feature_alpha'    : 0.5,
            }
        },
        {
            'name'  : 'dirichlet_label_dirichlet_feature.yaml',
            'config': {
                'partition'        : 'dual',
                'client_num'       : 20,
                'dir_path'         : 'domainnet',
        ...
        {
            'name'  : 'extreme_heterogeneity.yaml',
            'config': {
                'partition'        : 'dual',
                'client_num'       : 20,
                'dir_path'         : 'domainnet',
                ...
                'label_partition'  : 'dir',
                'label_alpha'      : 0.1,
                'feature_partition': 'single',
                'feature_alpha'    : 0.1,
            }
        }
    ]

    config_dir = Path('domainnet_configs')
    config_dir.mkdir(exist_ok=True)

    for config_info in configs:
        config_path = config_dir / config_info['name']
        with config_path.open('w') as f:
            yaml.dump(config_info['config'], f, default_flow_style=False)
        print(f"Created sample config: {config_path}")

    print(f"\nAll sample configs saved to: {config_dir}")
    print("Usage:")
    print("  cp domainnet_configs/<config_name> config.yaml")
    print("  python dataset/generate_domainnet.py")
