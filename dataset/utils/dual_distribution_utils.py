...
def split_dual_distribution(data: np.ndarray,
    ...
    # 打印详细统计信息
    _print_detailed_statistics(X, y, f, statistic, num_clients)

    # 如果是pat分布，额外打印全局分布信息
    if feature_partition == 'pat':
        _print_pathological_global_distribution(f, num_features, num_clients)
...
def _print_detailed_statistics(X: List, y: List, f: List, statistic: Dict, num_clients: int):
    """打印详细的client统计信息"""
    print("\n" + "=" * 80)
...
def _print_pathological_global_distribution(f: List, num_features: int, num_clients: int):
