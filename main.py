...
class FedSim:
    def __init__(self, args):
        ...
        # === init clients & server ===
        self.clients = [
            alg_module.Client(idx, args)
            for idx in tqdm(range(args.total_num), desc="Building clients")
        ]
        ...
        if self.suffix.exists():
            ...
            if existing_configs:
                ...
                log_cache = (
                    f"Resuming training under {self.suffix}..."
                    if can_resume
                    else f"\nConfig mismatch, creating new directory."
                )
            else:
                log_cache = (
                    f"No config found in {self.suffix}, starting new training run."
                )
            print(log_cache)
        ...
        config_target = (
            self.suffix
            / f"{args.alg}_{args.dataset}_{args.model}_{args.total_num}c_{args.epoch}E_lr{args.lr}.yaml"
        )
        ...
        logger_path = (
            self.suffix
            / f"{args.alg}_{args.dataset}_{args.model}_{args.total_num}c_{args.epoch}E_lr{args.lr}.log"
        )
        ...
        if can_resume:
            try:
                self.begin_round = self.load_checkpoint(
                    begin_round_idx=self.resume_round
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to resume training from {self.suffix}: {e}. Starting a new training run."
                )
    ...
    def simulate(self):
        ...
        try:
            for rnd in tqdm(
                range(self.begin_round, self.server.total_round),
                desc="Communication Round",
                initial=self.begin_round,
                total=self.server.total_round,
            ):
                # ===================== train =====================
                ...
                if isinstance(self.server, AsyncBaseServer):
                    ...
                    while True:
                        ...
                        # 检查是否有足够的客户端完成训练
                        if (
                            len(self.server.pending_aggregation_queue)
                            >= self.server.aggregation_batch_size
                        ):
                            break
                ...
                # ===================== save ckpt =====================
                if (CKPT_GAP > 0) and (
                    (self.server.total_round - rnd <= 10)
                    or (rnd % CKPT_GAP == (CKPT_GAP - 1))
                ):
                    _ = self.save_checkpoint(rnd)

                # ===================== test =====================
                if (self.server.total_round - rnd <= 10) or (
                    rnd % TEST_GAP == (TEST_GAP - 1)
                ):
                    ...
                    self.logger.info(
                        f"[Round {rnd}]\tAcc: {acc:.2f}\t| Time: {time:.2f}s"
                    )
            ...
            try:
                ...
                combined = set(zip(rnd_list, acc_list, time_list)) | set(
                    zip(recorded_rnd, recorded_acc, recorded_time)
                )
    ...
    def load_checkpoint(
        self,
        suffix_dir_path: str | Path | None = None,
        begin_round_idx: int | str = "max",
    ) -> int | None:
        """
        ...
        if begin_round_idx == "max":
            ...
            if begin_round_idx >= self.server.total_round:
                self.logger.error(
                    f"Checkpoint round {begin_round_idx} exceeds total rounds {self.server.total_round}."
                )
                raise ValueError(
                    f"Checkpoint round {begin_round_idx} exceeds total rounds {self.server.total_round}."
                )
        ...
        try:
            ...
            for client in self.clients:
                ...
                client.personalized_tensor2model(
                    client_states[client.id]["client_personalized_tensor"]
                )
...
def setup_logger(
    log_path: str | Path, name: str = "fed", level: int = logging.INFO
) -> logging.Logger:
