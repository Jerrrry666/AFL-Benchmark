...
class BaseClient:
    def __init__(self, id, args):
        ...
        self.dataset_train = get_dataset(self.dataset_name)(
            args, self.id, is_train=True
        )
        self.dataset_test = get_dataset(self.dataset_name)(
            args, self.id, is_train=False
        )
    ...
    def _model2tensor(model, personalized_flag):
        ...
        """
        if not personalized_flag:
            return None
        selected_param = [
            p.detach() for pick, p in zip(personalized_flag, model.parameters()) if pick
        ]
    ...
    def _tensor2model(tensor, model, personalized_flag):
        if not personalized_flag:
            return
        selected_params = [
            p for pick, p in zip(personalized_flag, model.parameters()) if pick
        ]
        with torch.no_grad():
...
class BaseServer(BaseClient):
    ...
    def sample(self):
        ...
        self.sampled_clients = sorted(
            random.sample(self.clients, sample_num), key=lambda x: x.id
        )
    ...
    def client_update(self):
        ...
        devices = (
            self.devices
            if isinstance(self.devices, list) and len(self.devices) > 0
            else ["cpu"]
        )

        # build a semaphore per device
        device_semaphores = {
            device: Semaphore(self.max_per_device) for device in devices
        }
        ...
        self.wall_clock_time += max(
            [client.training_time for client in self.sampled_clients]
        )
    ...
    def uplink(self):
        ...
        self.received_params = [
            nan_to_zero(client.model2shared_tensor()) for client in self.sampled_clients
        ]

    def aggregate(self):
        assert len(self.sampled_clients) > 0
        total_samples = sum(
            len(client.dataset_train) for client in self.sampled_clients
        )
        weights = [
            len(client.dataset_train) / total_samples for client in self.sampled_clients
        ]

        self.received_params = [
            params * weight for weight, params in zip(weights, self.received_params)
        ]
    ...
    def test_all(self):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            for client in self.clients:
                idx = client.id
                device = (
                    self.devices[idx % len(self.devices)] if self.devices else "cpu"
                )
