...
class Client(AsyncBaseClient):
    def __init__(self, id, args):
        ...
        self.mask = {
            name: torch.ones_like(param, dtype=torch.int)
    ...
    def generate_mask(self):
        for name, param in self.model.named_parameters():
            ...
            flat_mask = torch.cat([torch.zeros(num_zeros), torch.ones(num_ones)])
...
class Server(AsyncBaseServer):
    ...
    def aggregate(self):
        ...
        for mask, param, c_param in zip(
            self.cur_client.mask.values(),
            self.model.parameters(),
            self.cur_client.model.parameters(),
        ):
            mask_c = torch.ones_like(mask, dtype=torch.int) - mask
            param.data = (
                param.data * mask_c
                + ((1 - decay) * param.data + decay * c_param.data) * mask
            )
