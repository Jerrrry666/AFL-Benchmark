...
class Client(AsyncBaseClient):
    ...
    def train(self):
        ...
        # Line 12 - set h_prev
        if self.h is None:
            self.h = torch.zeros_like(s_grad)
        prev_h = self.h

        # Line 13 - solve zeta_grad
        if self.prev_s_grad is None:
            self.prev_s_grad = torch.zeros_like(s_grad)
...
class Server(AsyncBaseServer):
    def aggregate(self):
        ...
        model_g -= (zeta * len(self.cur_client.dataset_train)) / sum(
            len(c.dataset_train) for c in self.clients
        )
